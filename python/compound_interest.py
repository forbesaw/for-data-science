...
def userChoiceRate():
    ...
    if choice == 1:
        inflation_rate = 1.02 # and usd_rate = placeholder # will call API stored values? USD rate * number of years?
        return inflation_rate

    if choice == 2:
        inflation_rate = 1.0000 #and eur_rate = placeholder
        return inflation_rate

    if choice == 3:
        inflation_rate = 1.0075 #and gbp_rate = placeholder
        return inflation_rate

    if choice == 4:
        inflation_rate = 1.0000 #and btc_rate = placeholder
        return inflation_rate
...
for i in range(0, years):
    ...
    final_output = 'Final amount gained after {} years: '.format(years) + userChoiceCur() + '{:,}'.format(round(final_amount, 2)) + ' at a {:.2%} APR '.format(APR)
